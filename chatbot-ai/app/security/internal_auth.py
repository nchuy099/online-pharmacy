...
def decode_and_verify_internal_jwt(token: str) -> dict[str, Any]:
    ...
    secret = _env("CHATBOT_INTERNAL_JWT_SECRET")
    if not secret:
    ...
    expected_issuer = _env("CHATBOT_INTERNAL_JWT_ISSUER", "smart-pharma-backend")
    if payload.get("iss") != expected_issuer:
        raise InternalAuthError("Invalid internal token")

    expected_audience = _env("CHATBOT_INTERNAL_JWT_AUDIENCE", "chatbot-ai")
    aud = payload.get("aud")
    aud_values = aud if isinstance(aud, list) else [aud] if aud else []
    if expected_audience not in aud_values:
        raise InternalAuthError("Invalid internal token")

    if not payload.get("sub") or not payload.get("conversation_id"):
        raise InternalAuthError("Invalid internal token")
