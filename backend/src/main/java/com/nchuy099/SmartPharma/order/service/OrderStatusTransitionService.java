...
public class OrderStatusTransitionService {
    ...
    public void confirm(OrderEntity order) {
        transition(order, EnumSet.of(OrderStatus.PENDING), OrderStatus.PROCESSING,
                "confirm");
    }
    ...
    public void cancel(OrderEntity order) {
        transition(order, EnumSet.of(OrderStatus.PENDING, OrderStatus.PENDING_PAYMENT), OrderStatus.CANCELLED,
                "cancel");
    }
    ...
    public void syncFromGhn(OrderEntity order, String ghnStatus) {
        ...
        OrderStatus currentStatus = normalize(order.getStatus());
    ...
    private void assertTransitionAllowed(OrderEntity order, Set<OrderStatus> allowedSources, String actionName) {
        ...
        if (currentStatus == null) {
            throw new AppException(ErrorCode.CONFLICT,
                    "Cannot " + actionName + " order with missing status");
        }
